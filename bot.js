...
// Обработчик "Вернуться в начало"
bot.hears(['Вернуться в начало'], async (ctx) => {
  try {
    ctx.reply(messages.customerType, customerKeyboard());
  } catch (error) {
    await handleError(ctx, error, 'Ошибка при обработке выбора "Вернуться в начало"');
  }
});
